...
def glider_gun(grid, x, y):  # wykonanie pkt 4 - działo, wspolrzedne zywych komórek względem działka na siatce
    coords = [
        (0, 24),
        (1, 22), (1, 24),
    ...
    for dx, dy in coords:  # wstawia 1 (zywa komórka) w odpowiednim miejscu
        grid[x + dx][y + dy] = 1


def matuzalek(grid, x, y):  # wykonanie pkt 5 - matuzalek (dokładniej R-pentomino)
    coords = [(0, 1), (0, 2), (1, 0), (1, 1), (2, 1)]
    for dx, dy in coords:  # zaznacza wartość 1 w odpowiednim miejscu siatki, przesuniętym o (dx, dy) względem punktu startowego (x, y)
        grid[x + dx][y + dy] = 1


def animate(grid, steps,
            interval=200):  # animacja komorek w kazdej iteracji (interval to czas między kolejnymi klatkami animacji, domyślnie ustawiony na 200ms)
    fig, ax = plt.subplots()  # tworzenie obiektu wykresu
    img = ax.imshow(grid,
                    cmap='binary')  # wyswietla grid jako obrazek w czarno-białych kolorach (binary), komórki zywe - czarny, komórki martwe - biały

    text = ax.text(0.95, 0.95, '', transform=ax.transAxes,  # informacja ile zywych komorek
                   fontsize=12, color='red', ha='right', va='top',
                   bbox=dict(facecolor='white', alpha=0.7))

    def update(frame):  # funckaj wywołana dla kazdej klatki animacji
        nonlocal grid
        ...
        if frame + 1 == steps:  # zatrzymuje animacje jeśli to ostatnia określona epoka
            print(f"Końcowa liczba żywych komórek: {alive_cells}")
            ani.event_source.stop()

        return [img, text]

    ani = animation.FuncAnimation(fig, update, frames=steps, interval=interval, blit=False)  # tworzenie animacji
    plt.show()  # wyświetlanie animacji


def main():  # funkcja main() nadzorująca działanie programy i pozostałych funkcji, daje uytkownikowi mozliwosc wyboru symulacji
